document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener("submit", async e => {
        ...
        for (const c of campos) {
            ...
            if (val <= 0)
                return alert("Todos os campos devem ser maiores que zero.");
            novo[c] = val;
        }
        
        novo.nome = document.getElementById("nome").value.trim();
    ...
    async function render() {
        ...
        try {
            ...
            lista.forEach((orc) => {
                ...
                box.innerHTML = `<strong>Orçamento ${orc.nome || orc.id}</strong>`;
