...
(function () {
  ...
  function aplicarMascara(input) {
    ...
    input.addEventListener('blur', () => { input.value = formatar(input.value); });
    // valor inicial
    if (!input.value) input.value = formatar(0);
  }
  ...
  async function render() {
    ...
    lista.forEach(item => {
      ...
      if (row.querySelector('[data-col="nome"]')) row.querySelector('[data-col="nome"]').textContent = item.nome || '';
  ...
  btnExportar?.addEventListener('click', async () => {
    try {
      ...
      lista.forEach((o, idx) => {
        ...
        linhas.forEach(t => { doc.text(t, 42, y); y += 16; });
